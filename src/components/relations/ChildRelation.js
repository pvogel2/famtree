...
function ChildRelation(props) {
  ...
  useEffect(() => {
    ...
    if (treeLayout === 'rounded') {
      ...
      const sourceTest = new Cylindrical(sourceX, sourceY, sourceZ);

      lines = getRelationLinesCyl(sourceTest, target, parent, { foreground, highlight });
      const v = new Vector3();parent.getWorldPosition(v);
